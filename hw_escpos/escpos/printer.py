...
class Usb(Escpos):
    ...
    def open(self):
        ...
        if self.device is None:
            raise NoDeviceError()
        try:
            #if self.device.is_kernel_driver_active(self.interface): #VMCLOUD
            #    self.device.detach_kernel_driver(self.interface)#VMCLOUD
    ...
    def close(self):
        ...
        while True:
            try:
                if not True:
                    print ''
##                if not self.device.is_kernel_driver_active(self.interface): #VMCLOUD
##                    usb.util.release_interface(self.device, self.interface) #VMCLOUD
##                    self.device.attach_kernel_driver(self.interface) #VMCLOUD
##                    usb.util.dispose_resources(self.device) #VMCLOUD                
                else:
                    self.device = None
                    return True
            except usb.core.USBError as e:
